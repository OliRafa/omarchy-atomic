...
- [~] Phase 3 — build wiring (in progress):
  - [x] Wired `etc-files.sh` into `fedora-image-userland.sh` (Containerfile step 4b) — **fixes the original timezone bug**: the repo `etc/` tree (incl. `/etc/sudoers.d/omarchy-tzupdate`) now lands at image build. Made the sudoers `visudo` check delete only on an actual invalid report, never on visudo-absence (it isn't installed yet at step 4b), and added FATAL build guards for the tzupdate rule + `/etc/xdg/kitty`.
  ...
  - [x] Coverage: new static `image-userland-etc-test.sh` (tests.yml) + explicit assertions in `images/core/hack/smoke.sh` (built by the `core-image-e2e` job → real e2e validation of the wiring).
  - [ ] **Flag — cups-pdf:** `.core`/`.fedora` still ship `cups-pdf` (the root PDF backend the CUPS hardening removes; migration `1787815267` drops it on upgrade but fresh image installs keep it). Adding `cups-pk-helper` without removing `cups-pdf` leaves the image half-hardened — decide whether to drop `cups-pdf` from the image set.
  - [ ] **Flag — Claude Chromium extension:** `bin/omarchy-install-chromium-claude` seeds `/usr/share/{chromium,google-chrome,microsoft-edge}/extensions`, the **system**-Chromium external-extension path. This fork ships Chromium as a **Flatpak** (sandboxed; does not read /usr/share/*/extensions), so baking to /usr is likely inert. Needs the Flatpak per-app extension mechanism researched before wiring — deferred, not baked.
