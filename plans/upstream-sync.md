...
- [x] Phase 2 — platform adaptations (test debt cleared; a few upstream hardening ports remain deferred — update-lock/stay-awake orchestration and the hardened windows-vm compose writer — see the crash-fix "Deferred re-adds" note above).
...
- [x] Phase 4 — new software dispositions (green, e2e-validated):
  - [x] Dropped the desktop apps with no aarch64-Linux path — Claude Desktop, Hermes Desktop, Perplexity, ChatGPT — their install/remove commands + 8 menu entries + `theme-set-hermes` + the Hermes-Desktop skin migration + 4 desktop-only tests. Kept the Claude CLI + the Phase-3 Chromium extension, the Hermes CLI (mise) + `default-agent hermes`, and `theme-set-claude` (Claude *Code*).
  - [x] Ported T3 Code → `omarchy-mise-install npm:t3` and OpenClaw → `omarchy-mise-install npm:openclaw` (CLI/gateway/webapp); menu guards → `omarchy-cmd-present`, remove commands → mise teardown, OpenClaw webapp icon resolved from the mise install with a generic fallback.
  - Runtime-only risks flagged: T3 launches via `t3` (no `.desktop` app-grid entry under mise); OpenClaw's arm64 native-dep prebuilds resolve at first run.
- [x] Phase 5 — pacman purge (green, e2e-validated):
  - [x] Deleted pure-Arch commands (`omarchy-{refresh-pacman,refresh-pacman-mirrorlist,update-pacman,update-pkg-prune,pkg-publish-aarch64,dev-pkg-test,upgrade-to-quattro-mac,update-system-pkgs-when-conflicted}`, `build-packages.sh`), `default/pacman/` mirrorlists, the x86 `linux-omarchy`/limine migration `1789325478`, and their tests; converted the remaining live `pacman -Q/-Qo/-Qqo/-Rns` fallbacks to `rpm`/`dnf` (`omarchy-version`, `-debug`, `-upload-log`, `-remove-launcher-entry`, `-update-restart`).
  - [x] Removed the **legacy Arch-Mac disk-install cluster** (`omarchy-mac-setup`, `omarchy-system-btrfs-migrate`, `omarchy-system-boot-to-esp`) + `docs/btrfs.md` + their standalone `tests/` — dead on the bootc fork (install is the image + `deploy/`), reachable only from each other.
  - **Kept deliberately:** `omarchy-migrate`'s pacman-detection regex (the safety guard that self-skips Arch migrations), the self-skipping migrations, historical docs/conflicts, and — per the user's call — the Arch package manifests (`omarchy-base/other/unavailable.packages`) + `install/packaging/*` for **upstream-sync parity** (inert on Fedora; the build uses `.core`/`.fedora`).
  - The only remaining `pacman` string is the intentional detection regex in `omarchy-migrate`.

**All five phases complete. PR #18 is green through the full merge.**
