...
//
// The fork's omarchy-pkg-present is `rpm -q <name>`: a literal package-name
// lookup with no provides resolution -- `rpm -q sh` fails even though bash owns
// /bin/sh, and `rpm -q vim` fails with only gvim installed. Mirror that exactly
// with a name set from `rpm -qa`, or the shadow disagrees with the real command
// for every provided-but-not-owned name. A version constraint (`bash>=1`) is
// not a plain name, so it falls through to rpm itself; no shipped guard writes
// one.
function guardHelpers() {
  return 'declare -A __omarchy_pkgs=()\n'
    + "mapfile -t __omarchy_pkg_names < <(rpm -qa --qf '%{NAME}\\n' 2>/dev/null)\n"
    ...
    + '[[ $1 == *[\\<\\>=]* ]] && { rpm -q "$1" &>/dev/null; return; }; return 1; }\n'
